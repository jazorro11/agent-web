...
export const TOOL_CATALOG: ToolDefinition[] = [
  ...
  },
  {
    ...
    name: "read_file",
    description:
      "Reads an existing text file from the server filesystem. Use this when you need to inspect source code, config, logs, or any UTF-8 text without changing it. Do NOT use this to create or modify files; use write_file or edit_file instead. Do NOT use this if you only need a directory listing — this tool does not list folders. IMPORTANT: All paths are resolved within FILE_TOOLS_ROOT (the server's allowed directory). Always use relative paths (e.g. 'report.txt') — they resolve inside FILE_TOOLS_ROOT automatically. Absolute paths outside FILE_TOOLS_ROOT are rejected with PATH_OUTSIDE_ROOT error, which includes the allowed root in the message. Optional `offset` is the 1-based start line number (first line is 1). Optional `limit` is the maximum number of lines to return starting at `offset`. If both are omitted, the full file is returned up to a server-enforced maximum. Binary files are not supported. Process: resolve path → read from disk → slice by line range if requested → return JSON. Success: { ok: true, path, content, startLine, endLine, totalLines }. Failure: { ok: false, path, error: { code, message } } with explicit reason (e.g. file not found, file too large, tool disabled).",
    risk: "low",
    parameters_schema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    name: "edit_file",
    description:
      "Edits an EXISTING UTF-8 text file by replacing EXACTLY ONE occurrence of old_string with new_string. Use this when you need to update part of a file without rewriting the whole file. Do NOT use this to create a new file (use write_file). If old_string might match zero or multiple places, add more surrounding context to make it unique. old_string and new_string are literal substrings, not regex. Line endings must match those in the file. IMPORTANT: All paths are resolved within FILE_TOOLS_ROOT (the server's allowed directory). Always use relative paths (e.g. 'notes.txt') — they resolve inside FILE_TOOLS_ROOT automatically. Absolute paths outside FILE_TOOLS_ROOT are rejected with an error that includes the allowed root. Process: resolve path → read file → count occurrences of old_string → if not exactly 1, fail with a clear message (0 found vs N found) → apply replacement → write safely → return JSON. Success: { ok: true, path, replacements: 1 }. Failure: { ok: false, path, error: { code, message } } e.g. file not found, old_string found 0 times, old_string found N>1 times, permission denied, or tool disabled. Human approval required before execution.",
    risk: "high",
    parameters_schema: {
      type: "object",
      properties: {
